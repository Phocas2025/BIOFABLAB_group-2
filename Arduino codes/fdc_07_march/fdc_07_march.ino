...
void loop() {
    ...
    myStepper.run(); // Non-blocking stepper motor movement
    ...
    if (Serial.available() > 0) {
        ...
        if (input.equalsIgnoreCase("t")) {
            ...
            // Wait for motion completion without blocking the force measurement
            // This check ensures we don't block other actions in the loop
            while (myStepper.distanceToGo() != 0) {
                myStepper.run();  // Continue running the stepper until it's done
            }
