...
void loop() {
    ...
    const int serialPrintInterval = 50; // Interval for printing readings
    ...
    if (isMotorMoving && newDataReady && millis() > t + serialPrintInterval) {
        ...
        Serial.print(-displacement); // Correcting the sign of displacement
        Serial.println(" mm");
    ...
    if (Serial.available() > 0) {
        ...
        if (input.equalsIgnoreCase("t")) {
            ...
            long totalSteps = abs(X) / mmPerStep; // Use absolute value for step calculation

            // Move in the correct direction based on the sign of X (swapped directions)
            if (X > 0) {
                // Positive displacement: move counterclockwise (upward)
                myStepper.move(-totalSteps); 
            } else {
                // Negative displacement: move clockwise (downward)
                myStepper.move(totalSteps); 
            }
        }
    }
