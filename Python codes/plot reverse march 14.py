...
def parse_data(data):
    ...
    try:
        ...
        # Convert to float and reverse the sign of displacement
        force_formatted = float(force)
        displacement_formatted = -float(displacement)  # Negate the displacement value
...
def move_displacement(x):
    ...
    # Reverse the direction by negating the input displacement
    reversed_x = -x
...
def move_and_read(x):
    """ Move stepper by X mm, read force and displacement simultaneously, and auto-tare 0.5s after movement starts. """
    print(f"Moving to {x} mm displacement with a 3-second delay before starting.")
    time.sleep(3)  # 3-second delay before starting the movement
    ...
    try:
        with open('force_displacement_data.csv', mode='w', newline='') as file:
            ...
            # Wait 0.5 seconds after movement starts, then execute tare command
            time.sleep(0.5)
            tare()

            # Continuously read data while the movement is happening
            while True:
                data = read_serial()
                if data:
                    ...
                    if force is not None and displacement is not None:
                        # Print the corrected displacement value
                        ...
                        # Check if the motor has reached the target displacement
                        if abs(displacement - x) < 0.01:  # Tolerance for stopping
