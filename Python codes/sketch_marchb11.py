...
def move_displacement(x):
    ...
    if x == 0:  # Check if displacement is zero
        print("Error: Displacement cannot be zero.")
        return
    print(f"Moving by {x:.2f} mm (relative)")
    response = send_command(f"move {x}")  # Send relative displacement to Arduino
    if response:
        print(response)
        last_displacement += x  # Update the last reached position
    else:
...
def exit_program():
    ...
    send_command("stop")  # Send stop command to Arduino
