...
def move_and_read(x):
    ...
    stable_readings = 0  
    prev_motor_position = myStepper.currentPosition()  # Track the initial position of the motor
    motor_stopped = False  # Flag to track if the motor has stopped
            ...
            while True:
                ...
                if data:
                    ...
                    if force is not None and displacement is not None:
                        # ** If positive displacement, keep it positive on print and graph ** 
                        if x > 0:  # Moving clockwise
                        ...
                        # ** Check if motor stopped (based on steps taken) **
                        current_motor_position = myStepper.currentPosition()
                        if current_motor_position == prev_motor_position:
                            motor_stopped = True
                        prev_motor_position = current_motor_position

                        # ** Exit After 5 Consecutive Stable Readings AND Motor Stops ** 
                        if stable_readings >= 10 and motor_stopped:
