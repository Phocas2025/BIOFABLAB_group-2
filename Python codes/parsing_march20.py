...
# Track the last reached displacement
...
def read_serial():
    ...
    try:
        ...
        print(f"Raw serial data: {line}")  # Debugging: Print raw serial data
        return line
...
def parse_data(data):
    ...
    try:
        ...
        parts = data.split(", ")
        if len(parts) != 2:
            print(f"Unexpected data format: {data}")  # Debugging: Print unexpected data
            return None, None
        force_part, displacement_part = parts
...
def move_displacement(x):
    """ Move stepper by X mm (relative to last displacement). """
    global last_displacement
    new_target = last_displacement + x  # Ensure movement is relative
    print(f"Moving from {last_displacement:.2f} mm by {x:.2f} mm to {new_target:.2f} mm")
    response = send_command(str(new_target))
    print(response)
    last_displacement = new_target  # Update the last reached position
