...
data = pd.read_csv("force_displacement_data_5_04.csv")
...
if indenter_type == "sphere":
    ...
    # F = (4/3) * E* * sqrt(R) * ((delta - delta0) ** d) + F0
    ...
    lower_bounds = [0, -np.inf, 1, -np.inf]  # E_star ≥ 0, d ≥ 1
    upper_bounds = [np.inf, np.inf, 2, np.inf]  # d ≤ 2
